...
export default function EditSponsorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchSponsor = async () => {
      try {
        setSponsorId(resolvedParams.id);
        const data = await apiClient.sponsors.getById(resolvedParams.id);
        setFormData({
          name: data.name,
          image: data.image,
          link: data.link,
        ...
        router.push("/admin/customisations/sponsors");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.sponsors.update(sponsorId, formData);
      router.push("/admin/customisations/sponsors");
    } catch (error) {
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-navy font-bold uppercase tracking-widest">Loading...</div>;
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black uppercase text-navy">Edit Sponsor</h1>
          <p className="text-navy/60 font-bold">Update tournament sponsor details.</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="bg-white border-4 border-navy shadow-[8px_8px_0px_#0A192F] p-8 space-y-6">
        <div>
          <label className="block font-black text-navy uppercase tracking-wider mb-2">Sponsor Name</label>
          <input
            type="text"
            required
            value={formData.name}
        ...
        <div>
          <label className="block font-black text-navy uppercase tracking-wider mb-2">Logo</label>
          <input
            type="file"
            accept="image/*"
          ...
          />
          {formData.image && (
            <div className="mt-4 w-48 h-32 bg-navy/5 border-2 border-navy relative overflow-hidden">
        ...
        <div>
          <label className="block font-black text-navy uppercase tracking-wider mb-2">Website Link</label>
          <input
            type="url"
            required
            value={formData.link}
        ...
        <div>
          <label className="block font-black text-navy uppercase tracking-wider mb-2">Short Description</label>
          <textarea
            required
            value={formData.short_description}
            onChange={(e) => setFormData({ ...formData, short_description: e.target.value })}
            className="w-full bg-offwhite border-4 border-navy p-4 font-bold text-navy focus:outline-none focus:bg-white transition-colors h-32 resize-none"
          />
        </div>
