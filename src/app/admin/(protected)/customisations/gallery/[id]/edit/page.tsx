...
export default function EditGalleryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchGalleryItem = async () => {
      try {
        ...
        const data = await apiClient.gallery.getById(resolvedParams.id);
        setFormData({
          name: data.name,
          image: data.image,
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.gallery.update(galleryId, formData);
      router.push("/admin/customisations/gallery");
    } catch (error) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black uppercase text-navy">Edit Gallery Image</h1>
          <p className="text-navy/60 font-bold">Update details for this gallery image.</p>
        </div>
      </div>
